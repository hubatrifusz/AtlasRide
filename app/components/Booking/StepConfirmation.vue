  ...
  <div class="w-full max-w-4xl mx-auto">
    ...
    <div class="text-center mb-10">
      ...
      <p class="text-text-secondary">
        Kérjük, ellenőrizze a megadott adatokat a foglalás véglegesítése előtt
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="space-y-6">
        ...
        <fieldset
          ...
          <legend
            class="font-semibold text-lg text-accent-500 px-3 flex items-center gap-2">
          ...
          <div class="space-y-3">
            <div class="flex items-start gap-3">
              <UIcon
                ...
                class="text-accent-400 mt-1 shrink-0" />
              <div>
                <p class="text-text-primary font-medium">
                  {{ bookingStore.form?.departureLocation.city }}
                  <span
                    v-if="bookingStore.form?.departureLocation.zipCode"
                    class="text-text-secondary text-sm">
                    ({{ bookingStore.form?.departureLocation.zipCode }})
                  </span>
                </p>
                <p
                  v-if="bookingStore.form?.departureLocation.street"
                  class="text-text-secondary text-sm">
            ...
            <div class="flex items-center gap-3">
              ...
              <p class="text-text-primary">
                ...
                <span class="text-accent-400 font-semibold ml-2">{{
                  formatTime(bookingStore.form?.departureDateTime.time)
                }}</span>
              </p>
            </div>
          </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-text-primary font-medium">
                ...
                <span
                  v-if="bookingStore.form?.destinationLocation.zipCode"
                  class="text-text-secondary text-sm">
                  ({{ bookingStore.form?.destinationLocation.zipCode }})
                </span>
              </p>
              <p
                v-if="bookingStore.form?.destinationLocation.street"
                class="text-text-secondary text-sm">
        ...
        <fieldset
          ...
          <div class="flex items-center gap-3">
            ...
            <p class="text-text-primary">
              ...
              <span class="text-accent-400 font-semibold ml-2">{{
                formatTime(bookingStore.form?.returnInfo.returnDateTime?.time)
              }}</span>
            </p>
          </div>
        </fieldset>
        <div
          v-else
          class="border-2 border-main-700 bg-main-800/30 rounded-xl p-6">
        ...
        <fieldset
          ...
          <div class="flex items-center gap-3">
            <UIcon
              ...
              class="text-accent-400 shrink-0" />
            <p class="text-text-primary font-medium text-xl">
              {{ bookingStore.form?.passengerCount }} fő
            </p>
          </div>
        </fieldset>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-text-secondary text-sm">
                  {{
                    bookingStore.form?.departureLocation.isAirport
                      ? 'Induló járat'
                      : 'Érkezési járat'
                  }}
                </p>
                <p class="text-text-primary font-medium text-lg">
                  {{ bookingStore.form?.flightInfo?.outboundFlightNumber }}
                </p>
              </div>
            </div>
            <div
              v-if="
                bookingStore.form?.returnInfo.isReturn &&
                bookingStore.form?.flightInfo?.returnFlightNumber
              "
              class="flex items-center gap-3 pt-2 border-t border-main-600">
              <UIcon name="i-lucide-ticket" class="text-accent-400 shrink-0" />
              <div>
                <p class="text-text-secondary text-sm">
                  {{
      ...
      <div class="space-y-6">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-text-primary font-medium">
                ...
                <span
                  v-if="bookingStore.form?.customer.companyLocation?.zipCode"
                  class="text-text-secondary text-sm">
                  ({{ bookingStore.form?.customer.companyLocation?.zipCode }})
                </span>
              </p>
              <p
                v-if="bookingStore.form?.customer.companyLocation?.street"
                class="text-text-secondary text-sm">
        ...
        <!-- Comments -->
        <fieldset
          v-if="bookingStore.form?.comment"
          class="border-2 border-main-600 bg-main-800/50 rounded-xl p-6 hover:border-accent-500 transition-colors duration-300">
          <legend
            class="font-semibold text-lg text-accent-500 px-3 flex items-center gap-2">
    ...
    <!-- Summary Banner -->
    <div
      class="mt-8 bg-linear-to-r from-accent-700 to-accent-600 rounded-xl p-6 shadow-lg">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-accent-100 text-sm">
              Ellenőrizze a fenti információkat, majd kattintson a "Megerősítés"
              gombra
            </p>
          </div>
        </div>
