...
test.describe('Booking Page - E2E Tests', () => {
  ...
  test.describe('Page initialization', () => {
    test('has title', async ({ page }) => {
      await expect(page).toHaveTitle('Atlas Ride - Prémium reptértranszfer szolgáltatás');
    });

    test('it should be at the first step after initialization', async ({ page }) => {
      await expect(page.getByText('Utazás részletei')).toBeVisible();
    });
  });

  test.describe('Action buttons', () => {
    test('only Next button is visible after initialization', async ({ page }) => {
